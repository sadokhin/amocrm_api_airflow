import asyncio
import datetime
import pandas
import pandas_gbq
import requests
import time
import json
from google.cloud import bigquery
import google.auth
import logging

from airflow import models
from airflow.operators.python_operator import PythonOperator

# Step 1. Parameters. Edit it
SOURCE_NAME = 'statuses'
DAG_NUMBER = '1'

################################## Parameters. Don't edit it! ##################################

# Параметры подключения к BQ
LOCATION = 'europe-west3'
PROJECT = 'analytics-prod'
DATASET = 'raw_data_amocrm'
TABLE_PIPELINE = 'pipelines'
TABLE_STAGES = 'stages'
YESTERDAY = datetime.datetime.now() - datetime.timedelta(days=1)
MAX_RUN_ITERATIONS = 100

# Таблицы для хранения данных
TABLE_NAME_PIPELINE = f'{PROJECT}.{DATASET}.{TABLE_PIPELINE}'
TABLE_NAME_STAGES = f'{PROJECT}.{DATASET}.{TABLE_STAGES}'

AMO_REFRESH_TOKEN = {
    'client_id' : models.Variable.get('amocrm_client_id'),
    'client_secret' : models.Variable.get('amocrm_client_secret'),
    'grant_type' : 'refresh_token',
    'refresh_token' : models.Variable.get('amocrm_refresh_token'),
    'redirect_uri' : models.Variable.get('amocrm_redirect_uri')
}
AMO_DOMAIN = models.Variable.get('amocrm_domain')

credentials, project = google.auth.default(
    scopes=[
        "https://www.googleapis.com/auth/drive",
        "https://www.googleapis.com/auth/cloud-platform",
    ]
)
client = bigquery.Client(credentials=credentials,project=project)


default_args = {
    'owner': 'Composer Example',
    'depends_on_past': False,
    'email': [''],
    'email_on_failure': False,
    'email_on_retry': False,
}

################################## Parametrs. Don't edit it! ##################################
with models.DAG(
    dag_id=f'{DATASET}_{SOURCE_NAME}_dag_{DAG_NUMBER}',
    default_args=default_args,
    catchup=False,
    start_date=YESTERDAY,
    schedule_interval='0 2 * * *',
    tags = ['load','amocrm','api'],
) as dag:
    
    def optimize_df_to_bq(table_name, dataframe):
        import pandas as pd
        import numpy as np
        from decimal import Decimal

        table_info = bigquery.get_table(table=table_name)
        table_schema = table_info.schema
        schema_columns = {field.name: field for field in table_schema}

        dtype_map = {
            'INTEGER': 'Int64',
            'FLOAT': np.dtype(float),
            'NUMERIC': Decimal,
            'DECIMAL': Decimal,
            'BIGNUMERIC': Decimal,
            'BIGDECIMAL': Decimal,
            'TIME': pd.StringDtype(),
            'DATE': pd.StringDtype(),
            'DATETIME': "datetime64[ns]",
            'TIMESTAMP': "datetime64[s]",
            'BOOLEAN': "boolean",
        }

        for field_name, field in schema_columns.items():
            field_type = field.field_type
            dtype = dtype_map.get(field_type)

            if dtype:
                if field_type in ['NUMERIC', 'DECIMAL', 'BIGNUMERIC', 'BIGDECIMAL']:
                    dataframe[field_name] = dataframe[field_name].applymap(dtype)
                elif field_type in ['TIME', 'DATE', 'DATETIME', 'TIMESTAMP']:
                    dataframe[field_name] = pd.to_datetime(dataframe[field_name], errors='coerce',
                                                        infer_datetime_format=True)            
                else:
                    dataframe[field_name] = dataframe[field_name].astype(dtype, errors='ignore')
            else:
                dataframe.loc[:, field_name] = dataframe.loc[:, field_name].replace(
                    {pd.NA: None, pd.NaT: None, np.NaN: None}).apply(lambda x: str(x) if x else None)

        dataframe = dataframe[schema_columns.keys()]
        logging.info(f'{table_name}: All columns were formatted to BigQuery data types')
        return dataframe
        
    def _call_amo_method(method, data, headers):
        def _call_amo(method, data, headers):
            return requests.get(
                url='{}api/v4/{}'.format(AMO_DOMAIN, method),
                headers=headers,
                params=data,
                verify=True
            )

        response = None
        call_delay = 1
        while call_delay < 3:
            try:
                response = _call_amo(method, data, headers)
                if response.ok and response.json():
                    return response
            except:
                pass
            time.sleep(call_delay)
            call_delay += call_delay
        return response

    async def pipeline_list():
        iteration = 0
        # get access token using resfresh token (inactive resfresh token expire in 3 month)
        token_url = AMO_DOMAIN + 'oauth2/access_token'
        token_request = requests.post(token_url, data=AMO_REFRESH_TOKEN)
        request_dict = json.loads(token_request.text)
        print(token_request.status_code, token_request.text)
        access_token = request_dict['access_token']
        refresh_token = request_dict['refresh_token']

        #save value of new refresh token to airflow variable
        logging.info('access_token={}, refresh_token={}'.format(access_token, refresh_token))
        models.Variable.set('amocrm_refresh_token', refresh_token)
        headers = {'Authorization': 'Bearer ' + access_token}
        response = _call_amo_method('leads/pipelines', '', headers)
        data_resp = response.json()
        pipelines = data_resp['_embedded']['pipelines']
        while 'next' in data_resp.get('_links') and iteration < MAX_RUN_ITERATIONS:
            method = data_resp.get('_links').get('next').get('href').split('api/v4/')[1]
            response = _call_amo_method(method, '',headers)
            logging.info('next page: {} downloaded.'.format(method))
            data_resp = response.json()
            pipelines.extend(data_resp['_embedded']['pipelines'])
            iteration += 1
        return pipelines

    def load_statuses():
        pipelines = asyncio.run(pipeline_list())
        print(len(pipelines))
        statuses = []
        for status in pipelines:
            statuses.extend(status['_embedded']['statuses'])
        df_pipelines = pandas.json_normalize(pipelines)
        df_stages = pandas.json_normalize(statuses)
        print(df_stages)
        # Drop the original '_embeded' column and '_links' and 'custom_fields_values'
        df_pipelines.drop(['_embedded.statuses', '_links.self.href'], axis=1, inplace=True)
        df_stages.drop(['_links.self.href'], axis=1, inplace=True)    
        for i in df_pipelines.columns:
            print(i)
        for i in df_stages.columns:
            print(i)

        # Change dataframe type
        # df = optimize_df_to_bq(table_name=TABLE_NAME, dataframe=df)
        # logging.info('{}: optimized datatypes.'.format(TABLE_NAME))     

        pandas_gbq.to_gbq(df_pipelines, TABLE_NAME_PIPELINE, PROJECT, if_exists='replace')
        pandas_gbq.to_gbq(df_stages, TABLE_NAME_STAGES, PROJECT, if_exists='replace')

        return True

    update_statuses = PythonOperator(
        task_id='update_pipelines',
        python_callable=load_statuses,
    )

    update_statuses
